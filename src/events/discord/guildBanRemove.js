...
module.exports = {
  ...
  async execute(client, ban) {
    try {
      ...
      let reason = 'Tidak ada alasan yang dispesifikasikan.';
      ...
      try {
        ...
        client.logger.warn(`Gagal mengambil Audit Logs untuk Unban di guild ${ban.guild.id}: ${err.message}`);
      }

      const embed = new EmbedBuilder()
        .setColor('#2ecc71') // Hijau untuk Unban
        .setTitle('🔓 Blokir Member Dicabut (Unban)')
      ...
      client.logger.error('Error di event guildBanRemove:', error);
    }
  }
};
